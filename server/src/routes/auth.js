...
function audit(userId, eventType, message) {
  db.prepare("INSERT INTO audit_events (id, user_id, event_type, message, created_at) VALUES (?, ?, ?, ?, ?)").run(
    createId("evt"),
    userId,
    eventType,
    message,
...
authRouter.post("/register", async (req, res) => {
  ...
  audit(userId, "register", `${user.role} account created with status ${verificationStatus}`);
...
authRouter.post("/login", async (req, res) => {
  ...
  const token = signSession(user);
  setSessionCookie(res, token);
  audit(user.id, "login", `User logged in with status ${user.verification_status}`);
  return res.json({ message: "Login successful.", user: publicUser(user) });
});

authRouter.post("/logout", requireAuth, (req, res) => {
  audit(req.user.id, "logout", "User logged out");
...
authRouter.put("/profile", requireAuth, (req, res) => {
  ...
  audit(req.user.id, "profile_update", "User profile updated");
...
authRouter.post("/forgot-password", async (req, res) => {
  ...
  audit(user.id, "forgot_password", "Password verification code requested");
...
authRouter.post("/forgot-password/verify", async (req, res) => {
  ...
  audit(user.id, "reset_password", "Password reset completed after email verification");
...
authRouter.post("/reset-password", async (req, res) => {
  ...
  const passwordHash = await hashPassword(parsed.data.password);
  db.prepare("UPDATE users SET password_hash = ?, updated_at = ? WHERE id = ?").run(passwordHash, new Date().toISOString(), record.user_id);
  db.prepare("UPDATE password_reset_tokens SET used_at = ? WHERE id = ?").run(new Date().toISOString(), record.id);
  audit(record.user_id, "reset_password", "Password reset completed");
