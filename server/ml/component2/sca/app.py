...
from src.sca_predictor import (
    predict_component2_sca_v28_antalgic_hard_negative,
...
REPORT_DIR = PROJECT_ROOT / "07_reports" / "component_2_sca" / "inference"

EXTRACTOR_CANDIDATES = [
    PROJECT_ROOT / "04_video_extraction" / "run_component2_sca_clinical_video.py",
    PROJECT_ROOT / "run_component2_sca_clinical_video.py",
    PROJECT_ROOT / "src" / "run_component2_sca_clinical_video.py",
]

VIDEO_EXTENSIONS = ["*.mp4", "*.mov", "*.avi", "*.mkv", "*.MP4", "*.MOV", "*.AVI", "*.MKV"]

st.set_page_config(
    page_title="Component 2A SCA Gait Screening",
    page_icon="🧠",
    layout="wide",
...
st.markdown('<div class="main-title">🧠 Component 2A SCA Gait Screening Dashboard</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="subtitle">Upload a walking video or training-safe landmarks CSV to run the SCA V28 antalgic hard-negative screening model.</div>',
...
def result_emoji(result: dict) -> str:
    reliability = str(result.get("reliability", "")).lower()
    if reliability == "low":
        return "🟡"
    if result.get("sca_detected") is True:
        return "⚠️"
    if result.get("sca_tendency") is True:
        return "🟡"
    return "✅"
...
def show_result(result: dict) -> None:
    final_text = result.get("final_result", "Result unavailable")
    ...
    st.markdown(
        ...
        <div class="big-result">
            <div class="small-muted">Final Component 2A SCA Decision</div>
            <div class="big-value">{emoji} {final_text}</div>
    ...
    sca_probability = float(result.get("mean_probability", result.get("mean_probability", 0.0)))
    max_probability = float(result.get("max_probability", 0.0))
    positive_ratio = float(result.get("positive_ratio", 0.0))

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        card("SCA Detected", result.get("sca_detected", "N/A"))
    with c2:
        card("Reliability", result.get("reliability", "N/A"))
    with c3:
        card("Mean Probability", f"{sca_probability:.4f}")
    with c4:
        card("Pattern Strength", result.get("pattern_strength", "N/A"))

    st.markdown('<div class="section-title">SCA Probability Gauge</div>', unsafe_allow_html=True)
    st.progress(max(0.0, min(1.0, sca_probability)))
    st.caption(
        "Window threshold: "
        f"{result.get('window_probability_threshold', '0.45')} | "
    ...
    with left:
        ...
        model_rows = {
            ...
            "Final SCA result": result.get("final_result", "N/A"),
            "SCA detected": result.get("sca_detected", "N/A"),
            "SCA tendency": result.get("sca_tendency", "N/A"),
            "Mean probability": result.get("mean_probability", "N/A"),
            "V28 SCA probability": result.get("mean_probability", result.get("mean_probability", "N/A")),
            "Max probability": result.get("max_probability", "N/A"),
            "Top-3 mean probability": result.get("top3_mean_probability", "N/A"),
            "Positive count": result.get("positive_count", "N/A"),
            "Positive ratio": result.get("positive_ratio", "N/A"),
            "Detection source": result.get("detection_source", "N/A"),
        }
    ...
    with right:
        ...
        quality_rows = {
            ...
            "Direction source": result.get("direction_source", "N/A"),
            ...
            "FPS source": result.get("fps_source", "N/A"),
    ...
    elif str(result.get("reliability", "")).lower() == "low":
        st.warning(note)
    else:
        st.info(note)

    report_path = result.get("window_report_path", "")
    if report_path and Path(report_path).exists():
        with open(report_path, "rb") as file:
            st.download_button(
                "Download window-level SCA report CSV",
                file,
...
def run_predict_on_csv(csv_path: Path, direction: str | None = None, fps: float | None = None) -> dict:
    with st.spinner("Running Component 2A SCA V28 model..."):
...
with st.sidebar:
    ...
    st.info(
        "SCA V28 can infer walking direction and FPS automatically, but you can override them manually here."
    )
    ...
    if uploaded_video:
        ...
        if st.button("Extract landmarks and predict", type="primary"):
            ...
            if extractor is None:
                st.error("Extractor script not found. Add `run_component2_sca_clinical_video.py` into `04_video_extraction/`.")
            else:
